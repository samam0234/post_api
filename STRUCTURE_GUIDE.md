...
8. [요청 흐름 시나리오 (5가지)](#8-요청-흐름-시나리오-5가지)
...
| **구현된 API** | 게시글 CRUD + 목록 페이징·검색·정렬 |
| **DB 테이블** | `post`, `post_stat`, `attachment` (후자 둘은 API 미연결) |
...
└── app/                          # 애플리케이션 루트 패키지
    ...
    │   ├── __init__.py
    │   └── post_schema.py        # PostCreate, PostUpdate, PostItem, PostDetail, PagingInfo, PostListResponse
    │
    ├── repositories/             # DB 쿼리 계층
    │   ├── __init__.py
    │   └── post_repository.py    # INSERT, SELECT, UPDATE, DELETE
    │
    ├── services/                 # 비즈니스 로직 계층
    │   ├── __init__.py
    │   └── post_service.py       # 페이징, 404, 조회수, 검색·정렬, 수정·삭제
    │
...
| `Attachment` | `Attachment` | `True` (1:N) | `all, delete-orphan` | 첨부파일 목록 |
...
| `post_id` | Integer | FK → `post.id`, NOT NULL | 소속 게시글 |
...
`PostStat`·`Attachment`는 **테이블·관계만 정의**되어 있으며, Repository/Service/Router에는 아직 연결되지 않았습니다. (섹션 13 참고)
...
| `Update(post, changes)` | `changes` dict로 필드 수정 → `updated_at` 갱신 | `Post` |
| `delete(post)` | Post 삭제 → commit | `None` |
...
| `PostStat` / `Attachment` API | **미구현** (Model·관계만 정의) |
...
| `PostStat` / `Attachment` | Model·relationship만 정의. Repository/Service/Router **미연결** |
...
| `Post.Attachment` vs `back_populates` | Post 속성명 `Attachment`, Attachment의 `back_populates="attachments"` — **이름 불일치** |
| `Attachment.post_id unique=True` | 주석은 1:N이나 `unique=True`면 게시글당 첨부 1개(1:1)로 제한됨 |
| `post_schema.py` | `Optional`은 `PostUpdate`에서 사용 |
...
| `post_model.py` | Post·PostStat·Attachment 테이블 + 관계 정의 |
| `post_schema.py` | PostCreate/Update/Item/Detail, PagingInfo, PostListResponse |
| `post_repository.py` | INSERT, SELECT, UPDATE, DELETE + 검색·정렬 쿼리 |
| `post_service.py` | 페이징·404·조회수·검색·정렬·수정·삭제 비즈니스 로직 |
| `post_router.py` | `/posts` CRUD HTTP 엔드포인트 |
