...
app.include_router(auth_router) # 분리된 auth router 모듈의 APIRouter 객체를 app에 포함
...
mcp = FastApiMCP(
    app, # 도구로 변환할 MCP 앱
    name="게시판 API", # Claude (LLM)에 표시될 MCP 서버 이름
    description="게시판 등록/조회/목록/수정/삭제를 수행하는 MCP 도구",
    include_operations=[
        "create_post",
        "get_post",
        "list_posts",
